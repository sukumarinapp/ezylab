...
if (isset($_POST['submit'])) {
    ...
    $insert_finance_year = Insert('macho_finance_year', array(
    ...
    if (is_int($insert_finance_year)) {

        $notes = $_POST['description'].' Financial year added by ' . $user;
        $receive_id = '1';
...
if (isset($_POST['update'])) {
    $finance_id = Filter($_POST['id']);
    $update = Update('macho_finance_year', 'id', $finance_id, array(
        'description' => Filter($_POST['description']),
        'from_date' => to_sql_date($_POST['from_date']),
        'to_date' => to_sql_date($_POST['to_date']),
        'modified' => $modified
    ...
    if ($update) {
        ...
        $notes = $_POST['description'].' Financial year modified by ' . $user;
        $receive_id = '1';
        $receive_role_id = GetRoleOfUser($receive_id);
        InsertNotification($notes, $user_id, $role_id, $receive_role_id, $receive_id);
                        ...
                        <?php if ($PageAccessible['is_write'] == 1) { ?>
                            <div class="card-title pull-right">
                                <button class="btn btn-labeled btn-secondary" type="button" data-toggle="modal"
                                        data-target="#add_finance_year">Create New
                    ...
                    <div class="card-body">
                        <table class="table table-striped my-4 w-100" id="datatable1">
                            ...
                            <tr>
                                ...
                                <th class="thead_data">From Date</th>
                                <th class="thead_data">To Date</th>
                            ...
                            $FinanceYearCounts = count($FinanceYearResult);
                            if ($FinanceYearCounts > 0) {
                                foreach ($FinanceYearResult as $FinanceYearData) { ?>
                                    <tr>
                                        <td class="tbody_data"><?= ++$no; ?></td>
                                        <td class="tbody_data">&nbsp;<?= $FinanceYearData['description']; ?></td>
                                        <td class="tbody_data">&nbsp;<?= from_sql_date($FinanceYearData['from_date']); ?></td>
                                        <td class="tbody_data">&nbsp;<?= from_sql_date($FinanceYearData['to_date']); ?></td>
                                        <td class="tbody_data">&nbsp;<?= $FinanceYearData['created']; ?></td>
                                        <td class="tbody_data">&nbsp;<?= $FinanceYearData['modified']; ?></td>
                                        <td>
                                            <div class="btn-group">
                                                <?php if ($PageAccessible['is_modify'] == 1) { ?>
                                                    <button class="btn btn-info" type="button"
                                                            onclick="ModalEdit(<?php echo $FinanceYearData['id']; ?>);">
                                                        <i class="fa fa-edit"></i> Edit
                                                    </button>
                                                <?php }
                                                if ($PageAccessible['is_delete'] == 1) { ?>
                                                    <button class="btn btn-danger" type="button"
                                                            onclick="Delete('macho_finance_year','id',<?php echo $FinanceYearData['id']; ?>);">
...
<div class="modal fade" id="add_finance_year" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">Create New Financial Year</h4>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" name="from_date" id="from_date"
                                               data-date-format="dd-mm-yyyy" required tabindex="2">
                                    </div>
                                    <div class="form-group">
                                        <label class="col-form-label">To Date</label>
                                        <input class="form-control" type="text" name="to_date" id="to_date"
                                               data-date-format="dd-mm-yyyy" required tabindex="3">
                                    </div>
                                </div>
                                <div class="card-footer">
                                    <div class="clearfix">
                                        <div class="float-right">
                                            <button class="btn btn-primary" type="submit" name="submit" tabindex="4">
                                                Save
                                            </button>
...
<div class="modal fade" id="edit_finance_year" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
     aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h4 class="modal-title" id="myModalLabel">Update Financial Year Details</h4>
                <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    $("#to_date").change(function () {
        ...
        if ((Date.parse(endDate) <= Date.parse(startDate))) {
            ...
            document.getElementById("to_date").value = startDate;
        }
    });
</script>
<script>
    function ModalEdit(id) {
        $.ajax({
            type: "POST",
            url: "EditFinanceYear.php",
            data: {
                id: id
            },
            success: function (response) {
                $('#edit_body').html(response);
                $('#edit_finance_year').modal('show');
            }
        });
    }
    ...
    window.onload = function () {
        ...
        if (document.getElementById('insert_success')) {
            swal("Success!", "New Finance Year has been Added!", "success");
        }
        ...
        if (document.getElementById('update_success')) {
            swal("Success!", "Finance Year has been Updated!", "success");
        }
