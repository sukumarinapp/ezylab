...
$total = $things[0]->total;
$home_visit = $things[0]->home_visit;
$net_amount = $things[0]->net_amount;

$subtotal = new item('Subtotal', $total);
$home_visit = new item('Home Visit', $home_visit);
$total = new item('Total', $net_amount, true);

$items[0] = new items("Test", "Amount");
$items[1] = new items("", "");

for ($fl = 0; $fl < count($print_data_array); $fl++) {
    $things = $print_data_array[$fl];
    $things_count = count($things);
    for ($sl = 1; $sl < $things_count; $sl++) {
        $name = $things[$sl]->name;
        //$unit_price = $things[$sl]->unit_price;
        //$qty = $things[$sl]->qty;
        $amount = $things[$sl]->amount;

        $items[$i] = new items($name, $amount);
        $i++;
    }
}
